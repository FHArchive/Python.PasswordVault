...
''' Import sys to exit the program, random to generate a master key and io to
...
''' Prompts the user to choose what they want to do next (default is Add
Password)
'''
def choice():
...
''' Writes the passwords to the password file and exits the program (save and
quit)
'''
def myquit():
    out = ""
...
''' Prompts the user to select the index that they wish to remove and then
removes that password
'''
def remove():
    # Show the passwords
    decrypt_part()
...
''' Takes a password, encrypts it using the master password and writes it to
the passwords list
'''
def encrypt():
    global keystring
...
''' Runs decrypt_part (the bit that actually decrypts and then choice) - not
particularly good but saved me rewriting code
'''
def decrypt():
    decrypt_part()
    choice()

''' Takes all encrypted passwords from the list, decrypts and prints them
(password list stays encrypted)
'''
def decrypt_part():
    global keystring
    print("Passwords:")
